...
### PandaGPT
PandaGPT is a general-purpose instruction-following model that can both see and hear. It performs complex tasks such as detailed image description generation, story writing inspired by videos, and answering audio-based questions. PandaGPT combines multimodal encoders from ImageBind and large language models from Vicuna, supporting six modalities: text, image/video, audio, depth, thermal, and IMU. It excels in cross-modal tasks despite being trained with only aligned image-text pairs. For more details, visit the [PandaGPT website](https://panda-gpt.github.io/).
